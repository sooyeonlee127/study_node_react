...
const UploadProductPage = () => {
    const [product, setProduct] = useState({ // state: 작성한 것을 기억해주는 것
        title: '',
        description: '',
        price: 0,
        continents: 1,
    ...
    const userData = useSelector(state => state.user?.userData); // 누가 생성했는지 정보가 필요함
    const navigate = useNavigate()

    const handleChange = (event) => { 
        const { name, value } = event.target;
        setProduct((prevState) => ({
            ...prevState, // 원래 있던 객체의 값
            [name]: value // name에 원래 있던 값을 오버라이드 해주는 것
        }))
    }
    ...
    const handleSubmit = async (event) => {
        event.preventDefault(); // 페이지 refresh 되는 것을 막기
